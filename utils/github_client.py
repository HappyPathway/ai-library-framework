...
class GitHubClient:
    ...
    def __init__(
        ...
        if not self.token.startswith(('ghp_', 'github_pat_')):
            raise ValueError(
                "GitHub token appears to be invalid (wrong format)")
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logger.warning("%s (attempt %s/%s): %s",
                                   error_msg, attempt + 1, retries, str(e))
                    time.sleep(delay)
    ...
    def create_repository_from_template(
        ...
        logger.info(
            f"Created new repository: {new_repo_name} from template: {template_repo_name}")
        return new_repo
    ...
    def set_team_access(self, repo: Repository, team_slug: str, permission: str = "admin") -> None:
        ...
        try:
            ...
            logger.info("Granted %s access to team %s for repo %s",
                        permission, team_slug, repo.name)
        except GithubException as e:
            logger.error("Failed to set team access: %s", e)
            raise
    ...
    def write_file(
        ...
        try:
            ...
            try:
                ...
                logger.info("Created new file %s in repo %s", path, repo.name)
                return result["content"]
    ...
    def get_repository(
        ...
        try:
            ...
            logger.error("Error accessing repository %s: %s",
                         repo_name, str(e))
            raise
