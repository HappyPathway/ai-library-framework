...
class ProcessDevice(BaseDevice):
    ...
    def _run_device_process(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error in device process: {e}")
            raise
        finally:
            self._cleanup()
...
# Simplified ZMQ device class for specific use cases
class ZMQDevice:
...
# Register specific device classes that other modules might expect
ZMQForwarder = ThreadDevice  # Used for pub/sub pattern
ZMQStreamer = ThreadDevice   # Used for push/pull pattern
ZMQProxy = ThreadDevice      # Generic proxy device
